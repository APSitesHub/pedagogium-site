...
export const PolskiA02KahootForm = ({ destination }) => {
  ...
  const linksSchema = yup.object().shape({
    polskia0_2_1: yup.string().optional(),
    polskia0_2_2: yup.string().optional(),
    polskia0_2_3: yup.string().optional(),
    polskia0_2_4: yup.string().optional(),
    polskia0_2_5: yup.string().optional(),
  ...
  const handleLinksSubmit = async (values, { resetForm }) => {
    ...
    if (!emptyValues || confirmation) {
      const polskia0_2links = { polskia0_2: { links: {} } };
      for (const [key, value] of Object.entries(values)) {
        if (value && key !== 'replace') {
          polskia0_2links.polskia0_2.links[key] = value;
        } else {
          polskia0_2links.polskia0_2.replace = value;
        }
      }
      try {
        const response = await axios.patch(destination, polskia0_2links);
  ...
  return (
    <>
      <AdminPanelSection>
      <FormTitle>Polski A0_2</FormTitle>
        <Formik
